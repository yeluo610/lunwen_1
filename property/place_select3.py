...
class Rectangle:
    ...
    def contains(self, point):
        check_x = self.dXMin < point.lon <= self.dXMax,
        check_y = self.dYMin < point.lat <= self.dYMax
...
class Quadtree:
    ...
    def subdivide(self):
        ...
        north_east = Rectangle((x+x1)/2,(y+y1)/2,x1,y1)
        self.northeast = Quadtree(north_east, self.capacity)

        south_east = Rectangle((x+x1)/2,y,x1,(y+y1)/2)
        self.southeast = Quadtree(south_east, self.capacity)

        south_west = Rectangle(x,y,(x+x1)/2,(y+y1)/2)
        self.southwest = Quadtree(south_west, self.capacity)

        north_west = Rectangle(x,(y+y1)/2,(x+x1)/2,y1)
...
def getPOI(task,places):
    TTP=[]
    for place in places:
        distance=great_circle((place.lat,place.lon),(task.lon,task.lat))
        if distance < 10 and len(list(set(task.good).intersection(set(place.goods)))) > 0:
            TTP.append(place)
    return TTP
    ...
    for place in getPOI(POINTS[0][0],places):
        ...
        for i in range(1, len(POINTS[0])):
            for place2 in getPOI(POINTS[0][i],places):
                dis=great_circle((place.lat,place.lon),(place2.lat,place2.lon))
