...
# 变化时间问题
# 需要修改，删除工人要看shedule是否全部完成，但是呢又有一个问题，因为时间不是实时更新的，所以工人如果一直没满，工人就一直不能退出
# 真的没有地点会快很多
...
# 距离函数
# len(获得数据总数）,i开始位置，count：一次性获取数据量
    ...
    for task in tasks:
        ...
        if len(places) > 0:
            for j in range(len(places)):
                ...
                # if (distance < min_distance and len(
                #         list(set(task.good).intersection(set(places[j].goods)))) > 0 and distance < task.radius):
                #
